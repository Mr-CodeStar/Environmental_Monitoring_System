...
def get_environmental_data(lat: float, lon: float, start_date: str, end_date: str):
    ...
    # CRITICAL: Check if data exists before calling .median()
    s2_count = s2_col.size().getInfo()
    l8_count = l8_col.size().getInfo()

    if s2_count == 0 or l8_count == 0:
        raise Exception(f"No satellite passes found between {start_date} and {end_date}. Try a wider date range.")
...
def get_historical_dataset(lat, lon, years=10):
    poi = ee.Geometry.Point([lon, lat])
    ...
    for m in range(n_months):
        try:
            ...
            if s2_col.size().getInfo() > 0 and l8_col.size().getInfo() > 0:
                ...
                combined = ndvi.addBands([ndwi, ndbi, lst])
